...
private:
    ...
    // поля
    struct Dictionary_note { // 1 слово (с популярностью)
        std::string word = "";
    ...
    void DeleteData();	// очистим данные ОЗУ
...
private:
    ...
    // поля
    struct DictData	// структура для unordered_map
    {
    ...
    EasyMenu_Dictionary* opened_dict_ptr_;	// для быстрого доступа классу
